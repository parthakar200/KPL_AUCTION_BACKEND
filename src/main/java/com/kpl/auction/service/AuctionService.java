...
public class AuctionService {
    ...
    @Transactional
    public TeamDTO editTeam(String teamId, AddTeamRequest req) {
        Team t = teamRepo.findById(teamId)
            .orElseThrow(() -> new NoSuchElementException("Team not found: " + teamId));
        if (req.getName() != null && !req.getName().isBlank()) {
            boolean nameConflict = teamRepo.findAll().stream()
                .anyMatch(other -> !other.getId().equals(teamId) && other.getName().equalsIgnoreCase(req.getName()));
            if (nameConflict) throw new IllegalArgumentException("Team name already exists");
            t.setName(req.getName());
        }
        if (req.getColor() != null && !req.getColor().isBlank()) t.setColor(req.getColor());
        if (req.getPoints() > 0) {
            // Adjust pointsSpent if new budget is lower than what's been spent
            if (req.getPoints() < t.getPointsSpent()) t.setPointsSpent(req.getPoints());
            t.setPoints(req.getPoints());
        }
        return toTeamDTO(teamRepo.save(t));
    }
    ...
    public AuctionStateDTO placeBid(BidRequest req) {
        ...
        // No category restriction — teams can buy any number of Cat-A/B/C players
        if (t.getPlayerIdList().size() >= 15)
            throw new IllegalStateException("Team " + t.getName() + " already has the maximum 15 players.");
